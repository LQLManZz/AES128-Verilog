//=============================================================================
// Testbench: tb_aes128_uart_wrapper.sv
// Directory: uart_integration/aes128/
// Description: Self-checking testbench for aes128_uart_wrapper.
...
module tb_aes128_uart_wrapper;
    ...
    initial begin
        $dumpfile("tb_aes128_uart_wrapper.vcd");
        $dumpvars(0, tb_aes128_uart_wrapper);
    end
    ...
    initial begin
        ...
        $display("==========================================================");
        $display(" AES-128-GCM UART Wrapper Self-Checking Testbench");
        $display(" Reference: NIST SP 800-38D Appendix B (TC2 & TC3)");
        ...
        //-------------------------------------------------------------
        // Test 1: NIST SP 800-38D Appendix B - Test Case 2 (16B PT)
        //-------------------------------------------------------------
        $display("\n>>> TEST 1: NIST Appendix B - Test Case 2 (16B PT, AES-128)");
        begin
            ...
            send_packet(CMD_LOAD_KEY, key_tc2);
            verify_ack("LOAD_KEY (TC2)", RESP_ACK_KEY);

            send_packet(CMD_LOAD_IV, iv_tc2);
            verify_ack("LOAD_IV (TC2)", RESP_ACK_IV);

            send_packet(CMD_LOAD_PLAINTEXT, pt_tc2);
            verify_ack("LOAD_PLAINTEXT (TC2)", RESP_ACK_PLAINTEXT);
            ...
            if (rx_cmd === RESP_CIPHERTEXT && rx_ct.size() == 16) begin
                int ct_match = 1;
                for (int i = 0; i < 16; i++) if (rx_ct[i] !== exp_ct_tc2[i]) ct_match = 0;
                if (ct_match) $display("  [PASS] CIPHERTEXT (TC2) matched NIST specification.");
                else begin
            ...
            if (rx_cmd === RESP_AUTH_TAG && rx_tag.size() == 16) begin
                int tag_match = 1;
                for (int i = 0; i < 16; i++) if (rx_tag[i] !== exp_tag_tc2[i]) tag_match = 0;
                if (tag_match) $display("  [PASS] AUTH_TAG (TC2) matched NIST specification.");
                else begin
        ...
        //-------------------------------------------------------------
        // Test 2: NIST SP 800-38D Appendix B - Test Case 3 (64B PT)
        //-------------------------------------------------------------
        $display("\n>>> TEST 2: NIST Appendix B - Test Case 3 (64B Multi-block PT)");
        begin
            ...
            send_packet(CMD_LOAD_KEY, key_tc3);
            verify_ack("LOAD_KEY (TC3)", RESP_ACK_KEY);

            send_packet(CMD_LOAD_IV, iv_tc3);
            verify_ack("LOAD_IV (TC3)", RESP_ACK_IV);

            send_packet(CMD_LOAD_PLAINTEXT, pt_tc3);
            verify_ack("LOAD_PLAINTEXT (TC3)", RESP_ACK_PLAINTEXT);
            ...
            if (rx_cmd === RESP_CIPHERTEXT && rx_ct.size() == 64) begin
                int ct_match = 1;
                for (int i = 0; i < 64; i++) if (rx_ct[i] !== exp_ct_tc3[i]) ct_match = 0;
                if (ct_match) $display("  [PASS] CIPHERTEXT (TC3 - 64B) matched NIST specification.");
                else begin
            ...
            if (rx_cmd === RESP_AUTH_TAG && rx_tag.size() == 16) begin
                int tag_match = 1;
                for (int i = 0; i < 16; i++) if (rx_tag[i] !== exp_tag_tc3[i]) tag_match = 0;
                if (tag_match) $display("  [PASS] AUTH_TAG (TC3) matched NIST specification.");
                else begin
        ...
        #(CLK_PERIOD * 100);
        $display("\n==========================================================");
        $display(" VERIFICATION SUMMARY: aes128_uart_wrapper");
        $display("   Total Checks : %0d", test_count);
        $display("   Passed       : %0d", test_count - error_count);
        $display("   Failed       : %0d", error_count);
        $display("==========================================================");

        if (error_count == 0) $display(" *** ALL AES-128 TESTS PASSED SUCCESSFULLY ***\n");
