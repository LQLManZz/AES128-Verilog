//=============================================================================
// Module: aes_uart_wrapper.sv
// Description: Top-level integration wrapper connecting physical UART RX/TX
//              pins to the existing, unmodified AES_GCM hardware accelerator.
//
// Target: PolarFire SoC Discovery Kit (MPFS095T-1FCSG325E)
//
// Features:
//   - Parameterized clock and baud rate (zero clock-crossing fabric logic)
//   - Decouples UART byte speed from 100 MHz hardware AES pipeline
//   - Automatically coordinates CU handshakes (key, IV, AAD, PT, tag)
//   - Synthesizable for Libero SoC
//=============================================================================
...
module aes_uart_wrapper #(
    ...
    //-------------------------------------------------------------------------
    // Internal Signals: AES_GCM Core Interface
    //-------------------------------------------------------------------------
    logic         aes_load_key;
    logic         aes_load_iv;
    logic         aes_mode;
    ...
    // Support up to 16 blocks (256 bytes) each of AAD, PT, and CT
    logic [127:0] mem_aad [0:15];
    logic [3:0]   cnt_aad_blocks;

    logic [127:0] mem_pt  [0:15];
    logic [3:0]   cnt_pt_blocks;

    logic [127:0] mem_ct  [0:15];
    ...
    // Existing AES_GCM core (completely unmodified)
    AES_GCM u_aes_gcm (
        .clk                  (clk),
        .rst_n                (rst_n),
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Latch Streaming Blocks (AAD and Plaintext)
            ...
            // Acknowledge AAD & PT packet completion
            if (parser_cmd_load_aad_done) begin
                builder_send_ack_aad <= 1'b1;
            end
            if (parser_cmd_load_pt_done) begin
                builder_send_ack_pt <= 1'b1;
            end

            // Forward parser errors to builder
            ...
            // Clear buffer counters when new encryption sequence finishes
    ...
    //-------------------------------------------------------------------------
    // Capture Outputs from AES_GCM Core (Ciphertext & Tag)
    //-------------------------------------------------------------------------
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (wrap_state)
                WRAP_IDLE: begin
                    ...
                    if (parser_cmd_start_encrypt_done) begin
                        led_busy   <= 1'b1;
                        // Forward key (AES_GCM accepts 128-bit key) and IV
                        aes_cipher_key <= (reg_key_len == 6'd32) ? reg_key[255:128] : reg_key[127:0];
                        aes_iv_in      <= reg_iv;
                        aes_load_key   <= 1'b1;
                        aes_load_iv    <= 1'b1;
                ...
                WRAP_START_KEY_IV: begin
                    // Release pulses, wait for CU to expand key and compute H
                    wrap_state <= WRAP_WAIT_AAD_READY;
                end

                WRAP_WAIT_AAD_READY: begin
                    // CU raises AAD_ready when ready to accept AAD (or no_AAD)
                ...
                WRAP_WAIT_DATA_READY: begin
                    // CU raises data_ready when ready to accept Plaintext
                ...
                WRAP_WAIT_PIPELINE: begin
                    // Wait until all ciphertext blocks AND the tag have been captured
                ...
                WRAP_SEND_CT: begin
                    // Trigger packet builder to transmit CIPHERTEXT (0x90)
                    builder_send_ciphertext <= 1'b1;
                    builder_ct_len_bytes    <= {cnt_pt_blocks, 4'd0}; // pt_blocks * 16 bytes
                ...
                WRAP_SEND_TAG: begin
                    // Trigger packet builder to transmit AUTH_TAG (0x91)
