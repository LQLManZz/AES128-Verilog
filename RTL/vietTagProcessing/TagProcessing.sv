module TagProcessing (
    ...
    assign verify_pass = tag_process_valid && (tag == tag_ref_reg);
    
    always_ff @(posedge clk, negedge rst_n) begin: TagReg
        if(!rst_n)
            tag <= 128'h0;
        else begin
            if (ghash_finish && E_loaded)
                tag <= ghash_out ^ E_reg;
            else
                tag <= 128'h0;
        end
    ...
    always_ff @(posedge clk, negedge rst_n) begin: TagProcessValidReg
        if(!rst_n)
